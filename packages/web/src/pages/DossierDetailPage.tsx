...
export function DossierDetailPage(): React.JSX.Element {
  ...
  const { data: events = [] } = useQuery<TimelineEvent[]>({
    ...
    queryFn: async () => {
      ...
      const { data, error } = await supabase
        .from('dossier_events')
        .select('id, event_type, human_summary, extracted_data, extraction_confidence, created_at, email_id')
        .eq('dossier_id', id)
        .eq('user_id', user.id)
  ...
  if (isLoading || !dossier) {
    return (
      <div className="flex justify-center items-center py-24">
        <Icon icon={Loading03Icon} size={32} color="#7A7065" className="animate-spin" aria-label="Chargement" />
      </div>
    );
  }
    ...
    <>
      <div className="max-w-2xl mx-auto pb-16">
        ...
        <div className="px-4 pt-5 pb-4 border-b border-sesame-surface-muted">
          <div className="flex items-start gap-4">
            <div className="shrink-0 w-16 h-16 rounded-lg bg-sesame-surface-muted border border-sesame-text/20 flex items-center justify-center">
              {dossier.image_url ? (
                <img src={dossier.image_url} alt="" className="w-full h-full object-cover rounded-lg" />
              ) : (
                <Icon icon={typeIcon} size={28} color="#2A241F" strokeWidth={1.5} aria-hidden />
              )}
            </div>
            <div className="flex-1 min-w-0">
              {merchant && <p className="font-body text-sm text-sesame-text-muted mb-0.5">{merchant}</p>}
        ...
        <div className="px-4 space-y-8 pt-6">
          ...
          {dossier.dossier_type === 'trip' && <TripSections dossier={dossier} />}
          ...
          {dossier.dossier_type === 'reservation' && <ReservationSections dossier={dossier} />}
      ...
      <DeleteDialog
        ...
        onConfirm={() => { setDeleteOpen(false); deleteDossier.mutate(); }}
      />
    </>
  );
}
