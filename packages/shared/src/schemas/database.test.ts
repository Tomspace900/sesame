...
describe('DossierTypeSchema', () => {
  it('accepte toutes les valeurs valides', () => {
    const valeurs = ['purchase', 'trip', 'accommodation', 'subscription', 'reservation', 'other'];
...
describe('DossierEventSchema', () => {
  ...
  it('rejette extraction_confidence hors de [0, 1]', () => {
    expect(DossierEventSchema.safeParse({ ...validDossierEvent, extraction_confidence: 1.5 }).success).toBe(false);
    expect(DossierEventSchema.safeParse({ ...validDossierEvent, extraction_confidence: -0.1 }).success).toBe(false);
  });
...
describe('MailAccountSchema', () => {
  ...
  it('rejette une adresse email invalide', () => {
    const result = MailAccountSchema.safeParse({ ...validMailAccount, email_address: 'not-an-email' });
    expect(result.success).toBe(false);
  });
