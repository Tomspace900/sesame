...
const logger = createLogger("verify-telegram-pairing");
...
Deno.serve(async (req: Request) => {
  ...
  logger.info(`Verifying code for user ${user.id}`);
  ...
  if (lookupError) {
    logger.error("pairing lookup error:", lookupError);
  ...
  if (new Date(pairing.expires_at) < new Date()) {
    ...
    return jsonError(
      "Code expiré — envoie /start à nouveau dans Telegram",
      "CODE_EXPIRED",
      400,
    );
  }
  ...
  logger.success(
    `Successfully linked chat ${pairing.telegram_chat_id} to user ${user.id}`,
  );
