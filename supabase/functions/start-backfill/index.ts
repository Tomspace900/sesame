...
import { refreshAccessToken, listMessages } from '../_shared/gmail.ts';
...
Deno.serve(async (req: Request) => {
  ...
  if (!mail_account_id) return jsonError('mail_account_id required', 'MISSING_PARAM', 400);

  // Get mail account (verify it belongs to the authenticated user)
  const { data: mailAccount, error: accountError } = await supabase
    .from('mail_accounts')
    .select('id, user_id, access_token_encrypted, refresh_token_encrypted, token_expires_at, backfill_status')
  ...
  const expiresAt = mailAccount.token_expires_at ? new Date(mailAccount.token_expires_at) : new Date(0);
  ...
  for (let page = 0; page < MAX_PAGES; page++) {
    ...
    let result: { messages: Array<{ id: string }>; nextPageToken?: string; resultSizeEstimate?: number };
    try {
      result = await listMessages(accessToken, gmailQuery, nextPageToken, Math.min(PAGE_SIZE, remaining));
    } catch (err) {
  ...
  return jsonSuccess({ queued: totalQueued, total_estimate: totalEstimate ?? totalQueued });
});
