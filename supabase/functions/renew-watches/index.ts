...
const logger = createLogger("renew-watches");
...
Deno.serve(async (_req: Request) => {
  logger.info(`Starting check for expiring Gmail watches...`);
  // Find Gmail accounts whose watch expires within 2 days
  const expirationThreshold = new Date(Date.now() + 2 * 24 * 60 * 60 * 1000)
    .toISOString();

  const { data: accounts, error } = await supabase
    .from("mail_accounts")
    .select(
      "id, access_token_encrypted, refresh_token_encrypted, token_expires_at, watch_expiration",
    )
    .eq("provider", "gmail")
    .or(`watch_expiration.is.null,watch_expiration.lt.${expirationThreshold}`);

  if (error) {
    logger.error("Failed to fetch expiring accounts:", error);
    return new Response("Error", { status: 500 });
  }

  const accounts_list = accounts ?? [];
  logger.info(`Found ${accounts_list.length} accounts needing watch renewal`);
  ...
  for (const account of accounts_list) {
    try {
      ...
      renewed++;
      logger.info(
        `Renewed watch for account ${account.id}, expires ${expiration.toISOString()}`,
      );
    } catch (err) {
      failed++;
      logger.error(`Failed to renew watch for account ${account.id}:`, err);
    }
  }
