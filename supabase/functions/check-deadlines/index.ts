...
function buildReturnMsg(d: DossierRow): string {
  ...
  return (
    ...
    `Tu as jusqu'au <b>${formatDate(d.return_deadline!)}</b> pour retourner cet article (${dayStr}).`
  );
}
...
function buildWarrantyMsg(d: DossierRow): string {
  ...
  return (
    ...
    `La garantie expire le <b>${formatDate(d.warranty_deadline!)}</b> (${dayStr}).`
  );
}
...
async function fetchCandidates(
  ...
  sentCol: 'return_reminder_sent' | 'warranty_reminder_sent' | 'renewal_reminder_sent',
  nowISO: string,
  windowEndISO: string,
): Promise<DossierRow[]> {
  let q = supabase
  ...
  if (error) {
    console.error(`fetchCandidates(${deadlineCol}) error:`, error);
    return [];
  }
...
Deno.serve(async (req: Request) => {
  ...
  const windowEndISO = new Date(now.getTime() + 60 * 24 * 60 * 60 * 1000).toISOString();

  // Fetch all 3 deadline lists in parallel
  const [returnCandidates, warrantyCandidates, renewalCandidates] = await Promise.all([
    fetchCandidates('return_deadline', 'return_reminder_sent', nowISO, windowEndISO),
    fetchCandidates('warranty_deadline', 'warranty_reminder_sent', nowISO, windowEndISO),
    fetchCandidates('next_renewal_at', 'renewal_reminder_sent', nowISO, windowEndISO),
  ]);

  const allCandidates = [...returnCandidates, ...warrantyCandidates, ...renewalCandidates];
  ...
  type DeadlineTask = {
    ...
    sentKey: 'return_reminder_sent' | 'warranty_reminder_sent' | 'renewal_reminder_sent';
  ...
  for (const task of tasks) {
    for (const dossier of task.candidates) {
      ...
      const thresholdDays = (prefs?.[task.thresholdKey] as number | undefined) ?? task.defaultDays;
      ...
      if (markError) {
        console.error(`mark ${task.sentKey} error (dossier ${dossier.id}):`, markError);
        continue;
      }
      ...
      try {
        ...
        console.error(`Telegram send failed (${task.deadlineKey}, dossier ${dossier.id}):`, err);
